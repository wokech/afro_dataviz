...
# 
# # Save the data
# write.csv(avocado_prod, "sub_pro_7_agriculture_owid/datasets/avocado-production-tonnes.csv",
#           row.names = FALSE)
  ...
  avocado_prod_clean_region <- avocado_prod_clean |>
    filter(is.na(code)) |>
    select(c(1,3,4)) 
  
  # Filter by FAO region
  
  avocado_prod_clean_region_fao <- avocado_prod_clean_region |>
    filter(grepl('(FAO)', region))
  
  # Filter by non-FAO region
  
  avocado_prod_clean_region_non_fao <- avocado_prod_clean_region |>
    filter(!grepl('(FAO)', region))
  
  # 3) Continental (Non-FAO) avocado production
  
  # a) Stacked area chart
  
  avocado_prod_clean_region_non_fao_continent <- avocado_prod_clean_region_non_fao |>
  ...
  avocado_prod_clean_region_non_fao_continent <- avocado_prod_clean_region_non_fao_continent |>
    mutate(region = factor(region, levels = desired_order)) |>
    arrange(desc(region))
  
  #  calculate cumulative positions for label placement
  
  label_df_avocado <- avocado_prod_clean_region_non_fao_continent |>
    filter(year == max(year)) |>
    mutate(region = factor(region, levels = rev(desired_order))) |>
    arrange(region) |>
    mutate(x_label = max(year),
           y_top = cumsum(avocado_production_tonnes),
           y_bottom = y_top - avocado_production_tonnes,
           y_mid = (y_bottom + y_top) / 2) |>
    select(region, year, x_label, y_top, y_mid) 
  
  # plot the stack area chart
  
  p1 <- avocado_prod_clean_region_non_fao_continent |> 
    ggplot(aes(year, avocado_production_tonnes, fill = region, label = region, color = region)) +
    geom_area() +
    geom_text_repel(
      data = label_df_avocado,
    ...
    scale_y_continuous(limits = c(0, 11000000), labels  = 
  ...
  avocado_prod_clean_region_non_fao_continent |>
    filter(year == 2020) |>
    mutate(percent = 100 * avocado_production_tonnes/sum(avocado_production_tonnes))
  ...
  label_df_avocado_percent <- avocado_prod_clean_region_non_fao_continent |>
    group_by(year) |>
    mutate(share = avocado_production_tonnes / sum(avocado_production_tonnes, na.rm = TRUE)) |>
    ungroup() |>
    filter(year == max(year)) |>
    mutate(region = factor(region, levels = rev(desired_order))) |>
    arrange(region) |>
  ...
  p2 <- avocado_prod_clean_region_non_fao_continent |> 
    ggplot(aes(year, avocado_production_tonnes, fill = region, color = region)) +
    geom_area(position = "fill") +
    geom_text_repel(
      data = label_df_avocado_percent,
      aes(x = x_label, y = y_mid, label = region, color = region),
      hjust = 0,
      fontface = "bold",
      size = 8,
  ...
  ggsave("sub_pro_7_agriculture_owid/images/continental_combi/avocado.png", width = 12, height = 16, dpi = 300)
  ...
  # 4) Avocado production in Africa
  
  # Organize data
  
  # Africa total combined with regions data
  avocado_prod_clean_region_fao_africa <- avocado_prod_clean_region_fao |>
    filter(str_detect(region, "frica"))
  
  # Africa data alone
  avocado_prod_clean_region_fao_africa_only <- avocado_prod_clean_region_fao_africa |>
    filter(region %in% c("Africa (FAO)"))
  
  # Africa regions alone
  avocado_prod_clean_region_fao_africa_segment <- avocado_prod_clean_region_fao_africa |>
  ...
  avocado_prod_clean_region_fao_africa_segment <- avocado_prod_clean_region_fao_africa_segment |>
    mutate(region = factor(region, levels = desired_order)) |>
    arrange(desc(region))
  
  label_df_avocado_africa <- avocado_prod_clean_region_fao_africa_segment |>
    filter(year == max(year)) |>
    mutate(region = factor(region, levels = rev(desired_order))) |>
    arrange(region) |>
    mutate(x_label = max(year),
  ...
  p3 <- avocado_prod_clean_region_fao_africa_segment |> 
    ggplot(aes(year, avocado_production_tonnes, fill = region, label = region, color = region)) +
    geom_area() +
    geom_text_repel(
      data = label_df_avocado_africa,
      aes(x = x_label, y = y_mid, label = region, color = region),
      hjust = 0.1,
      fontface = "bold",
      size = 8,
    ...
    theme_classic() +
    scale_x_continuous(breaks = c(1960, 1980, 2000, 2020), labels = c("1960", "1980", "2000", "2020")) +
    scale_y_continuous(limits = c(0, 1500000), labels  = 
  ...
  #ggsave("sub_pro_7_agriculture_owid/images/continental_africa_only/continent_avocado_1.png", width = 12, height = 12, dpi = 300)
  
  ################################################################################
  # Stacked Percentage Area Chart
  ################################################################################
  
  label_df_avocado_percent_africa <- avocado_prod_clean_region_fao_africa_segment |>
    group_by(year) |>
    mutate(share = avocado_production_tonnes / sum(avocado_production_tonnes, na.rm = TRUE)) |>
    ungroup() |>
    filter(year == max(year)) |>
  ...
  p4 <- avocado_prod_clean_region_fao_africa_segment |> 
    ggplot(aes(year, avocado_production_tonnes, fill = region, color = region)) +
    geom_area(position = "fill") +
    geom_text_repel(
      data = label_df_avocado_percent_africa,
      aes(x = x_label, y = y_mid, label = region, color = region),
      hjust = 0.1,
      fontface = "bold",
      size = 8,
  ...
  #ggsave("sub_pro_7_agriculture_owid/images/continental_africa_only_stack_perc/continent_avocado_1.png", width = 12, height = 12, dpi = 300)
  
  
  (p3/p4) + plot_annotation() & theme(plot.margin = margin(0,0,0,0))
  ggsave("sub_pro_7_agriculture_owid/images/continental_africa_only_combi/avocado.png", width = 12, height = 16, dpi = 300)