...
p2 <- oil_palm_prod_clean_region_non_fao_continent |> 
  ...
  geom_text_repel(
    data = label_df_oil_palm_percent,
    aes(x = x_label, y = y_mid, label = region, color = region),
    hjust = 0,
    fontface = "bold",
    size = 8,
...
# 4) Oil Palm production in Africa
...
p3 <- oil_palm_prod_clean_region_fao_africa_segment |> 
  ...
  theme_classic() +
  scale_x_continuous(breaks = c(1960, 1980, 2000, 2020), labels = c("1960", "1980", "2000", "2020")) +
  scale_y_continuous(limits = c(0, 3600000), labels  = 
...
p4 <- oil_palm_prod_clean_region_fao_africa_segment |> 
  ...
  geom_area(position = "fill") +
  geom_text_repel(
    data = label_df_oil_palm_percent_africa,
    aes(x = x_label, y = y_mid, label = region, color = region),
    hjust = 0.1,
    fontface = "bold",
    size = 8,
