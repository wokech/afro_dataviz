...
aquaculture_clean_region_wb %>% 
  ...
  labs(x = "Year",
       y = "Share of Aquaculture Production (%)",
       title = "",
       caption = "") +
...
# 4) Aquaculture production in Africa

# Organize data

# Africa total combined with regions data
aquaculture_clean_region_fao_africa <- aquaculture_clean_region_fao %>%
  filter(str_detect(region, "frica"))

# Africa data alone
aquaculture_clean_region_fao_africa_only <- aquaculture_clean_region_fao_africa %>%
  filter(region %in% c("Africa (FAO)"))

# Africa regions alone
aquaculture_clean_region_fao_africa_segment <- aquaculture_clean_region_fao_africa %>%
...
aquaculture_clean_region_fao_africa_segment <- aquaculture_clean_region_fao_africa_segment %>%
  mutate(region = factor(region, levels = desired_order)) %>%
  arrange(desc(region))

label_df_aquaculture_africa <- aquaculture_clean_region_fao_africa_segment %>%
  filter(year == max(year)) %>%
  mutate(region = factor(region, levels = rev(desired_order))) %>%
  arrange(region) %>%
  mutate(x_label = max(year),
         y_top = cumsum(aquaculture_production_tonnes),
         y_bottom = y_top - aquaculture_production_tonnes,
         y_mid = (y_bottom + y_top) / 2) %>%
  select(region, year, x_label, y_top, y_mid) 

# b) Stacked Area chart for Africa regions

aquaculture_clean_region_fao_africa_segment %>% 
  ggplot(aes(year, aquaculture_production_tonnes, fill = region, label = region, color = region)) +
  geom_area() +
  geom_text_repel(
    data = label_df_aquaculture_africa,
    aes(x = x_label, y = y_mid, label = region, color = region),
    hjust = 0.1,
    fontface = "bold",
    size = 8,
...
ggsave("sub_pro_7_agriculture_owid/images/continental_africa_only/continent_aquaculture_1.png", width = 12, height = 12, dpi = 300)

################################################################################
# Stacked Percentage Area Chart
################################################################################

label_df_apple_percent_africa <- aquaculture_clean_region_fao_africa_segment %>%
  group_by(year) %>%
  mutate(share = aquaculture_production_tonnes / sum(aquaculture_production_tonnes, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(year == max(year)) %>%
  mutate(region = factor(region, levels = rev(desired_order))) %>%
  arrange(region) %>%
...
aquaculture_clean_region_fao_africa_segment %>% 
  ggplot(aes(year, aquaculture_production_tonnes, fill = region, color = region)) +
  geom_area(position = "fill") +
  geom_text_repel(
...
ggsave("sub_pro_7_agriculture_owid/images/continental_africa_only_stack_perc/continent_aquaculture_1.png", width = 12, height = 12, dpi = 300)
