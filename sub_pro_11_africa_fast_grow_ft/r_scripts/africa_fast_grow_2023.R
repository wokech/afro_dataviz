...
table_data_2023_clean_country_treemap <- table_data_2023_clean |>
  group_by(country) |>
  summarise(total = n()) |>
  arrange(desc(total)) |>
...
ggplot(table_data_2023_clean_country_treemap, 
...
table_data_2023_clean_sector_treemap <- table_data_2023_clean |>
  mutate(sector = recode(sector, "Real estate" = "Real Estate")) |>
  mutate(sector = recode(sector, "E-commerce" = "E-Commerce")) |>
...
ggplot(table_data_2023_clean_sector_treemap, 
       aes(area = percent_total, fill = percent_total, 
           label = paste0(group, "\n",
                          percent_total, "%"))) +
  geom_treemap() +
...
table_data_2023_clean_year_group_treemap <- table_data_2023_clean_year_group |>
  group_by(founding_year_group) |>
  summarise(total = sum(total)) |>
  mutate(percent_total = round((total/sum(total))*100, 1)) |>
  arrange(desc(percent_total))
...
ggplot(table_data_2023_clean_year_group_treemap, 
       aes(area = percent_total, fill = percent_total, 
...
# List top 5 counties
top_5 <- africa |>
  filter(admin %in% c("Nigeria", "South Africa", "Egypt", "Namibia", "Kenya")) |>
  pull(admin)
